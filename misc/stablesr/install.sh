...
wget https://raw.githubusercontent.com/neuralninja22/colab/master/misc/stablesr/config.json -O /content/$1/config.json
wget https://raw.githubusercontent.com/neuralninja22/colab/master/misc/stablesr/ui-config.json -O /content/$1/ui-config.json
...
sed -i -e "/    token_merging_ratio_hr = p.get_token_merging_ratio(for_hr=True)/a\    if len(all_seeds) - 1 < index:\n        return \"\"" /content/$1/modules/processing.py
