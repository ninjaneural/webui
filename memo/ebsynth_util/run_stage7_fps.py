...
def search_out_dirs(proj_dir, blend_rate, preoutput):
    ### create out_dirs
    p = re.compile(r'.*[\\\/]'+preoutput+r'([0-9]+)[\\\/]')

    number_of_digits = -1
    
    out_dirs=[]
    for d in glob.glob( os.path.join(proj_dir ,preoutput+"*/"), recursive=False):
        print(d)
        m = p.fullmatch(d)
        if m:
...
def ebsynth_utility_stage7(dbg, project_args, blend_rate,export_type,is_invert_mask,preoutput,fps):
    dbg.print("stage7")
    dbg.print("")
    ...
    number_of_digits, out_dirs = search_out_dirs( project_dir, blend_rate, preoutput )
...
if __name__=='__main__':
    print("# make movie from frames")
    print("python run_stage7.py [mp4 file] [output folder prefix] [fps]")
    print("ex) python run_stage7.py input.mp4 out- 30")
    dbg = debug_string()
    argument = sys.argv
    if len(argument) > 3:
        ebsynth_utility_stage7(dbg, ("./", argument[1], '','','','',''), 1, "mp4", False, argument[2], argument[3])
