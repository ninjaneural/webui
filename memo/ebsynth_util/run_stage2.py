...
def ebsynth_utility_stage2(dbg, project_args, key_min_gap, key_max_gap, key_th, key_add_last_frame, is_invert_mask, zerofill):
    dbg.print("stage2")
    dbg.print("")
    ...
    for k in keys:
        filename = str(k).zfill(zerofill) + ".png"
...
if __name__=='__main__':
    print("# select keyframes")
    print("python run_stage2.py [mp4 file] [Input frames path] [Keyframe save path] [Min Keyframe gap] [Max Keyframe gap] [Threshold of delta frame edge] [Zero Fill]")
    print("ex) python run_stage2.py input.mp4 ./input ./video_key 10 300 8.5 7")
    dbg = debug_string()
    argument = sys.argv
    if len(argument) > 7:
        ebsynth_utility_stage2(dbg, ("./", argument[1], argument[2],'',argument[3],'',''), int(argument[4]), int(argument[5]), float(argument[6]), True, False, int(argument[7]))
