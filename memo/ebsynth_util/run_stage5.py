...
def export_project( project, proj_filename, preoutput ):
    ...
    with open(proj_path, 'wb') as f:
        ...
        for synth in project["synth_list"]:
            ...
            # out path
            path =  preoutput + str(synth["key"]).zfill( project["number_of_digits"] ) + project["file_name"]
...
def ebsynth_utility_stage5(dbg, project_args, is_invert_mask, preoutput):
    dbg.print("stage5")
    dbg.print("")
    ...
    for i, synth in enumerate(synth_list):
        ...
        if (i % SYNTHS_PER_PROJECT == SYNTHS_PER_PROJECT-1):
            ...
            export_project( project, proj_file_name, preoutput )
            proj_index += 1
            tmp = []
...
if __name__=='__main__':
    ...
    print("python run_stage5.py [Input frames path] [Keyframe save path] [output folder prefix]")
    print("ex) python run_stage5.py ./input ./key out-")
    dbg = debug_string()
    argument = sys.argv
    if len(argument) > 3:
        ebsynth_utility_stage5(dbg, ("./", '', argument[1], '', '', argument[2], ''), False, argument[3])
